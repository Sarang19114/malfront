  ...
  return (
    <div className="w-full " {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
      className="p-10 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden  bg-gray-900/[0.1] border-2 border-gray-400/70 group  shadow-lg">
        ...
        <div className="flex flex-col items-center justify-center">
          <p className="relative font-mono z-20 font-bold text-white text-3xl">
            Upload file
          </p>
          <p className="relative z-20 font-sans font-normal text-white text-2xl mt-2">
...
export function GridPattern() {
  ...
  useEffect(() => {
    const cellSize = 40; // Assuming each cell is 40px (w-10 translates roughly to 40px)
    
    const updateGridDimensions = () => {
      const newColumns = Math.ceil(window.innerWidth / cellSize);
  ...
  return (
    <div className="flex bg-neutral-900 flex-wrap">
      {Array.from({ length: rows }).map((_, row) =>
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              className={`w-10 h-10 flex rounded-[2px] ${
                index % 2 === 0
                  ? "bg-neutral-950"
